...
const LoanRequest = () => {
  ...
  const handleAccept = async (request) => {
    const customerId = request.customerId.integerValue;
    ...
    const loansCollectionUrl = `https://firestore.googleapis.com/v1/projects/${PROJECT_ID}/databases/(default)/documents/Loans/${customerId}?key=${API_KEY}`;
    ...
    try {
      ...
      if (loansResponse) {
        ...
        await axios.patch(loansCollectionUrl, {
          fields: {
            customerid: { integerValue: customerId }, // Document ID
            loans: {
              arrayValue: {
      ...
      setMessage('Loan request accepted successfully!');
